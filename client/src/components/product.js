...
export default function Product() {
  ...
  useEffect(() => {
    if (!Qty) dispatch(productList());
    // let Total =
    //   cartItems &&
    //   cartItems.reduce((total, itm) => {
    //     return total + Number(itm.price);
    //   }, 0);
    setTotal(totalPrice);
  }, [Qty]);
    ...
    <div className={classes.container}>
      ...
      {Qty ? (
        <div className={classes.cartBox}>
          ...
          <Link style={{ textDecoration: "none" }} to="/buy/address">
          <Button
            type="submit"
            variant="contained"
            color="primary"
          >
            {`Proceed to Buy  ( ${Qty}  item )`}
          </Button>
            </Link>
